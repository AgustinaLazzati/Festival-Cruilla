...
-------
  ...
  pip install matplotlib soundfile scikit-learn  # optional: plots + extra audio formats
...
-----
  ...
  report.plot_heatmap("diversity_heatmap.png")
  report.plot_space_3d("diversity_space_3d.png")
  ...
  # 3. CLI
  python diversity_metrics.py A.wav B.wav C.wav D.wav --backend encodec --plot-space-3d space.png
"""
...
class DiversityReport:
    ...
    # Store raw embeddings for 3D spatial mapping
    ...
    def plot_heatmap(self, save_path: Optional[str] = None) -> None:
        ...
        cmap = LinearSegmentedColormap.from_list("div", ["#ef4444", "#fbbf24", "#22c55e"])
    ...
    def plot_space_3d(self, save_path: Optional[str] = None) -> None:
        """Projects embeddings into 3D space using PCA and plots them."""
        try:
            import matplotlib.pyplot as plt
            from sklearn.decomposition import PCA
        except ImportError:
            warnings.warn("matplotlib or scikit-learn not installed. pip install matplotlib scikit-learn")
            return

        n = len(self.embeddings)
        if n < 3:
            print("Not enough points to plot a meaningful 3D space (need at least 3). Falling back to 2D logic if possible.")
            
        # Extract 3 components for 3D mapping
        X = np.stack(self.embeddings)
        n_components = min(3, n)
        pca = PCA(n_components=n_components)
        coords = pca.fit_transform(X)

        # Pad with zeros if data dimensions are insufficient
        if coords.shape[1] == 2:
            coords = np.pad(coords, ((0, 0), (0, 1)), mode='constant')
        elif coords.shape[1] == 1:
            coords = np.pad(coords, ((0, 0), (0, 2)), mode='constant')
        ...
        # 3D Scatter plot
        ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], c="#22c55e", s=180, edgecolors=BG, alpha=0.9, zorder=3)

        # Add text labels in 3D space
        short_labels = [Path(lb).stem[:30] for lb in self.labels]
        for i, txt in enumerate(short_labels):
            ax.text(coords[i, 0], coords[i, 1], coords[i, 2], f"  {txt}", color=TEXT, fontsize=10, zorder=4)

        # Formatting 3D axes
        ax.set_title(f"3D Embedding Space [{self.backend.upper()}]", color=TEXT, fontsize=14, pad=15)
        ax.set_xlabel("PC 1", color="#888")
        ax.set_ylabel("PC 2", color="#888")
        ax.set_zlabel("PC 3", color="#888")
        
        # Dark mode pane styling
        ...
        if save_path:
            ...
            print(f"[Diversity] 3D Space Plot saved → {save_path}")
        else:
            plt.show()
        plt.close()
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description="Pairwise embedding diversity & spatial mapping for audio."
    )

    p.add_argument(
        "files",
        nargs="*",
        help="Audio files to compare. If empty, uses outputs/music/metrics/*.wav",
    )
    ...
    p.add_argument(
        "--plot-heatmap",
        default=None,
        help="Save heatmap plot to this path (e.g. diversity_heatmap.png)",
    )
    
    p.add_argument(
        "--plot-space-3d",
        default=None,
        help="Save 3D PCA spatial map to this path (e.g. diversity_space_3d.png)",
    )
    ...
    if args.plot_heatmap:
        report.plot_heatmap(args.plot_heatmap)
        
    if args.plot_space_3d:
        report.plot_space_3d(args.plot_space_3d)